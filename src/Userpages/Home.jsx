...
function Home() {
  ...
  async function getFilter(id) {
    console.log("reached get filter");
    console.log(id);
    const fproducts = await api.get(`filter/${id}`)
    console.log(fproducts);
    setFproducts(fproducts.data.filterd)
    ...
    <UserLayout>
      <div className=" bg-[#f2f0f1] w-screen h-[100vh] flex flex-col sm:flex-row justify-around ">
        <div className=" w-[50%] text-center h-full flex items-center justify-center flex-col ">
          ...
          <button className=" bg-black text-white px-3 py-2 rounded-4xl my-4 text-[16px]">
            Shop Now
          </button>
      ...
      <div className="py-11 flex overflow-x-auto scroll-smooth  gap-10">
        {products.map((ele, ind) => {
          return (
            <div
              key={ind}
              className=" hover:scale-105 transition-all flex-shrink-0 gap-5 max-w-[200px]  min-w-[200px]  rounded-lg p-4 text-center shadow-[8px_3px_10px_5px_rgba(0,_0,_0,_0.1)] o"
              onClick={() => {
                navigate(`/product/${ele._id}`);
              }}
            >
              <img
                src={`http://localhost:3000/${ele.productImage}`}
              ...
              <button
                ...
                className="mt-3 bg-gray-800 text-white px-4 py-2 rounded hover:bg-gray-600 hover:scale-95 transition-all hover:text-shadow-black"
              >
                show More
              </button>
            </div>
      ...
      <div>
        ...
        <div className="py-11 flex overflow-x-auto scroll-smooth  gap-10">
          ...
          <img src={`http://localhost:3000/${items.productImage}`} className="mx-auto h-40 object-contain" />
